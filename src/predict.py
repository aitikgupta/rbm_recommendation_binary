...
def predict_output(model, train_folds, liked, disliked, top_like=3, top_dislike=2, verbose=True):
    ...
    like = []
    dislike = []
    _ = [like.append((i+1, probs[i]))
         for i in range(len(outs)) if outs[i] == 1]
    _ = [dislike.append((i+1, probs[i]))
         for i in range(len(outs)) if outs[i] == 0]
    ...
    if verbose:
        ...
        _ = [print(f"MovieID: {i[0]}, Confidence: {i[1]}")
             for i in dislike[:min(top_dislike, len(dislike))]]
...
if __name__ == "__main__":
    ...
    like, dislike = predict_output(
        model, train_folds, liked, disliked, top_like=5, top_dislike=3)