            ...
            for l in lr:
                for h in nh:
                    ...
                    for f in range(len(train_folds)):
                        losses = rbm.fit(train_folds[f], epochs=e, batch_size=32, lr=l, k_walk=k, verbose=False)
                        test_loss = rbm.test(train_folds[f], test_folds[f], verbose=False)
                        print(f"{test_loss} at Epochs: {e}, K_walks:{k}, LR: {l}, N_Hidden_Nodes: {h}, Fold: {f}")
